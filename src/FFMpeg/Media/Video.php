...
class Video extends Audio
{
    ...
    public function save(FormatInterface $format, $outputPathfile)
    {
        ...
        foreach ($filters as $filter) {
            if ($filter instanceof ConcatFilter) {
                $concatFilter = $filter;
            }
        }

        if ($concatFilter) {
            $commands = ['-y', '-i', $concatFilter->getFile(), '-i', $this->pathfile];
        } else {
        ...
        if ($this->driver->getConfiguration()->has('ffmpeg.threads')) {
            $filters->add(
                new SimpleFilter(['-threads', $this->driver->getConfiguration()->get('ffmpeg.threads')])
            );
        }
        ...
        if (null !== $failure) {
            throw new RuntimeException('Encoding failed: ' . $failure->getMessage(), $failure->getCode(), $failure);
        }
